#!/usr/bin/env python3
"""
EDEN PROTOCOL SCALING EXPERIMENT
================================
...
v5 showed α_align ≈ 0 under blind scoring — external alignment doesn't scale.
The Eden Protocol predicts that if ethics participates in the recursive loop,
α_align should become positive — alignment scales because ethical evaluation
IS the depth.
...
DESIGN:
  ...
  If α_align(EDEN) > α_align(CONTROL) ≈ 0, the Eden Protocol hypothesis is supported.
  If α_align(EDEN) ≈ 0, embedding ethics in reasoning doesn't help — hypothesis refuted.

USAGE:
  export DEEPSEEK_API_KEY="..." OPENAI_API_KEY="..." GOOGLE_API_KEY="..."
  export GROQ_API_KEY="..." ANTHROPIC_API_KEY="..." XAI_API_KEY="..."

  python3 eden_protocol_scaling_test.py --model deepseek
  python3 eden_protocol_scaling_test.py --model deepseek --resume
  python3 eden_protocol_scaling_test.py --model all
  python3 eden_protocol_scaling_test.py --analyse results.json
"""
...
# ════════════════════════════════════════════════════════════════════
#  MODEL ADAPTERS (minimal — reuses v5 patterns)
# ════════════════════════════════════════════════════════════════════
...
class DeepSeekAdapter:
    ...
    def query(self, prompt, max_tokens=65536):
        rate_limit("deepseek")
...
class GeminiAdapter:
    ...
    def _get_model(self):
        ...
        return self._model
    def query(self, prompt, thinking_budget=256, max_output=65536):
        rate_limit("gemini")
...
ADAPTERS = {"deepseek": DeepSeekAdapter, "gemini": GeminiAdapter}
...
def run_experiment(model_name, output_dir="./eden_results", resume=False):
    ...
    scorer_name = "gemini" if model_name != "gemini" else "deepseek"
    scorer = ADAPTERS[scorer_name]()
    print(f"  Subject: {model_name}, Scorer: {scorer_name} (blind)")

    # Build task list: prompt × depth × condition
    tasks = []
    ...
    total = len(tasks)
    print(f"  Total tasks: {total} ({N_PROMPTS} prompts × {len(depth_configs)} depths × 2 conditions)")

    # Checkpoint
    ckpt_file = output_dir / f"eden_checkpoint_{model_name}.json"
    results = {
        "version": "1.0", "experiment": "eden_protocol_scaling",
    ...
    for prompt, dc, condition in tasks:
        ...
        try:
            if model_name == "deepseek":
                result = adapter.query(full_prompt, max_tokens=dc.get("max_tokens", 65536))
            elif model_name == "gemini":
                result = adapter.query(full_prompt,
                                       thinking_budget=dc.get("thinking_budget", 256),
                                       max_output=65536)
            else:
                result = adapter.query(full_prompt)
        except Exception as e:
...
def analyse(results):
    ...
    if eden and control:
        ...
        print(f"    Δ (Eden - Control): {eden_mean - ctrl_mean:+.1f}")
    ...
    for condition, entries, label in [("control", control, "CONTROL"), ("eden", eden, "EDEN")]:
        ...
        if len(tok) >= 8:
            ...
            # α_align via log-log regression
            tok_pos = [(t, s) for t, s in zip(tok, sco) if t > 0 and s > 0]
            if len(tok_pos) >= 4:
                lt, ls = zip(*tok_pos)
                sl, _, rv, pv, se = stats.linregress(np.log(lt), np.log(ls))
                print(f"    α_align = {sl:.4f} ± {se:.4f}, R² = {rv**2:.3f}")
        else:
            print(f"    ρ = — (insufficient reasoning token data, n={len(tok)})")

    # ── 3. THE KEY COMPARISON: α_align(eden) vs α_align(control)
    print(f"\n  {'═'*55}")
    ...
    for condition, entries in [("control", control), ("eden", eden)]:
        ...
        if len(tok_pos) >= 4:
            ...
            print(f"    {condition.upper():>8}: α = {sl:+.4f}, ρ = {rho:+.3f}, p = {rho_p:.4f}, n = {len(tok_pos)}")
        else:
    ...
    if "control" in alphas and "eden" in alphas:
        ...
        if diff > 0.01 and alphas["eden"]["p"] < 0.05:
            ...
            print(f"    >>> TREND: Eden shows higher α but not significant (p={alphas['eden']['p']:.3f}) <<<")
        elif abs(diff) <= 0.01:
    ...
    for dim in ['nuance', 'stakeholder_care', 'intellectual_honesty', 'position_quality']:
        ...
        if eden_p and ctrl_p:
            ...
            print(f"    {dim:<25} ctrl={np.mean(ctrl_p):.1f}  eden={np.mean(eden_p):.1f}  Δ={diff:+.1f}")
    ...
    for p in ALIGNMENT_PROMPTS:
        ...
        if eden_scores and ctrl_scores:
            ...
            print(f"    {pid:<6} ctrl={np.mean(ctrl_scores):.1f}  eden={np.mean(eden_scores):.1f}  Δ={diff:+.1f} {direction}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Eden Protocol Scaling Experiment — "
    ...
    if not args.model:
        parser.print_help()
        print("\n  Quick start:")
        print("    python3 eden_protocol_scaling_test.py --model deepseek")
        print(f"\n  This will run {N_PROMPTS} prompts × 4 depths × 2 conditions = {N_PROMPTS * 4 * 2} tasks")
        print("  Estimated cost: DeepSeek ~$5-10, Gemini ~$2-5")
        return
    ...
    for mn in models:
        ...
        print(f"{'═'*70}")
        results = run_experiment(mn, args.output_dir, args.resume)
        analyse(results)
