...
====================================
Experimental validation using either the shared Eden gateway or the legacy
DeepSeek R1 API fallback.
...
Requirements:
- Python 3.8+
- EDEN_GATEWAY_URL and EDEN_GATEWAY_API_KEY for the preferred path
- openai >= 1.0.0 for legacy direct runs only
- DEEPSEEK_API_KEY for legacy direct runs only

Usage:
    export EDEN_GATEWAY_URL="https://your-eden-gateway.example.com"
    export EDEN_GATEWAY_API_KEY="your-key-here"
...
try:
    from archive.validation.eden_gateway import (
        EdenGatewayError,
        EdenGatewayOpenAIClient,
        extract_chat_text,
        load_eden_gateway_config,
    )
except ImportError:
    print("ERROR: Eden gateway helper is unavailable.")
    sys.exit(1)
...
def create_client():
    """Create and return the configured API client."""
    try:
        eden_config = load_eden_gateway_config()
    except EdenGatewayError as exc:
        print(f"ERROR: {exc}")
        sys.exit(1)

    if eden_config is not None:
        print("Using Eden gateway via EDEN_GATEWAY_URL and EDEN_GATEWAY_API_KEY.")
        return EdenGatewayOpenAIClient(eden_config)

    if not os.environ.get("DEEPSEEK_API_KEY"):
        print(
            "ERROR: Set either EDEN_GATEWAY_URL and EDEN_GATEWAY_API_KEY, "
            "or DEEPSEEK_API_KEY for legacy direct runs."
        )
        sys.exit(1)
    ...
    try:
        ...
        print("ERROR: openai package not installed for legacy direct runs.")
...
def call_deepseek(client, question, max_tokens=4096):
    ...
    try:
        ...
        # DeepSeek R1 provides reasoning_content separately. Eden responses may
        # only provide content, so we treat reasoning content as optional.
        reasoning_text = getattr(message, 'reasoning_content', '') or ''
        answer_text = getattr(message, 'content', '') or extract_chat_text(response)
