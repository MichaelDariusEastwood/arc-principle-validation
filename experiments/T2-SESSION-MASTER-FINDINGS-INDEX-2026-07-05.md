...
| L6 | **Overstatement (Paper III "mathematical certainty")** — universalises metabolic α=d/(d+1) to ALL physics (earthquakes/cosmology); corpus otherwise hedges well | A | flagged (substantive scope) | T5/operator | Scope to network-transport + soften "certainty"; finalise w/ F23 | OVERSTATEMENT-PASS / f3e08f8 |
...
- **Falsification criteria (F1-F29) honestly reported** — 13 distinct criteria, 23 fail/tension vs 26 pass vs 17 pending; open falsifications (F2, F7 falsified; F3 fails; F1a weakened; F10 untested). Corpus falsifies itself in print = deepest anti-crank signal. Open: consolidate into one canonical status table (T5/author).
...
## α=2.24 retraction consistency — VERIFIED EXEMPLARY (2026-07-05)
Follow-through on the book↔papers equation finding: does the ceiling-EXCEEDING α=2.24 (2.24 > α=2 ARC Bound) appear un-retracted anywhere? Checked all 8 papers that mention it. RESULT: retracted/revised HONESTLY in every one. My initial 'Paper III/IX no retraction context' flag was a FALSE ALARM (narrow regex — they use 'does not replicate'/'revise'/'single-model artefact' not 'retracted'). Paper III: 7 hits all revision-context ('KEY REVISION... does not replicate universally'). Paper IX EXEMPLARY: dedicated §7.1 'The Original Alpha Was a Single-Model Artefact' + §7.8 'The Alpha=2.24 Point Estimate Exceeded the ARC Bound' — openly documents the ceiling-breach + overclaim + correction. This is model anti-crank honesty (LAW 1). STRENGTHENS the book↔papers finding: papers rigorously retract the ceiling-breaching 2.24, so the book's U=I×R² framing is the outlier needing the bridge/errata. Book OMITS 2.24 (safe — no false claim; 2nd-ed retraction narrative = operator call). Verified positive; corrected own false alarm.
