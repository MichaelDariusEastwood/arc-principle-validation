...
import os as _os
# the figure is written beside this experiment's own results, never to a local folder
_FIGURE_OUT = _os.path.join(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))), 'results', 'blind_prediction_results.png')
_os.makedirs(_os.path.dirname(_FIGURE_OUT), exist_ok=True)
...
if len(results) > 0:
    ...
    plt.tight_layout()
    plt.savefig(_FIGURE_OUT,
                dpi=150, bbox_inches='tight', facecolor='white')
    print(f"  Figure saved: {_FIGURE_OUT}")
