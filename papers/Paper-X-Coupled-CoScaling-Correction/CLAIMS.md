...
| 2 | Exact steady-state misalignment fraction d* = gamma*r/(A+r), reducing to rho = gamma*r/A when A >> r. | **P** | Theorem 1; §3.5 |
| 3 | In the gain-only model the fraction **saturates at gamma, it does not diverge to infinity** (corrects the prior draft). | **P** | Theorem 2; §3.4 |
| 4 | A finite-time hard takeoff is alignment-stable iff beta > k; the verdict = sign(beta - k), independent of speed and of the singularity time. | **P** | Theorem 3 (Hard-Takeoff Coordinate-Artefact); §3.7 |
| 5 | Genuine divergence lives in a distinct compounding channel with threshold rho_prop = (gamma3 - 1)r/A < 1. | **P** | Theorem 4; §3.8 |
...
| 9 | The numerical implementation matches the closed-form derivation (10/10 internal-consistency + integrator checks). | **V** | code/experiment_coscaling.py; results/verdicts.json; test_coscaling.py |
| 10 | The beta/k estimator recovers known exponents from model-generated data (to ~0.1) and fails honestly on saturated data. | **S** | experiments/scripts/estimate_exponents.py; results/realmodel/exponent_estimates.json |
| 11 | On a real model (Claude), the external corrector removes a seeded reward-hack: blind D 10 -> 0, capability C 0 -> 1.0. | **Pilot** (mechanism; **IV.d non-compliant** - same-family scoring) | results/realmodel/claude-opus_corrector_probe.json; REAL_MODEL_CLAUDE_RESULTS.md |
| 12 | The co-scaling **dynamic** (H1 decoupled drifts up; H2 coupling bounds d) on a real model. | **NOT shown** (null contrast; the model did not drift) | results/realmodel/claude-opus_2026*.json (H1=false, H2=false) |
