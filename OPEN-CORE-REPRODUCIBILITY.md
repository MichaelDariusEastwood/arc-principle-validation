...
- Experiment/validation code: `experiments/` (copied 2026-06-15 from the website repo), `archive/validation/*.py`, `archive/research-toolkits/`.
- Scaling toolkit: `arc-scaling-challenge` (α/β estimators, falsification criteria, protocols) - already public-ready.
- `CITATION.cff` (added today → gives a "Cite this repository" button), MIT `LICENCE` (present), OSF DOI link (present in README badges).
...
## Prior art & honest attribution (this is what protects credibility)
...
- **Creation theory / recursion-as-creation / fine-tuning** have substantial prior art (Teilhard, Wheeler, Smolin, complexity theory) - frame ARC as a named *synthesis*, not a first.
...
## Licence
MIT for code (matches the SFF dissemination plan). Data under CC-BY recommended. README carries the Sharma citation.
