...
- Gaps/confusions found:
  - The runners for all three experiments are now committed under papers/Paper-VIII-The-Load-Bearing-Proof/experiments/. Seven of the deposited files are published with a change to their text, in two classes: one header comment line removed from the two portability patches, and dash characters replaced by punctuation in all seven. Each carries the sha256 of the bytes that ran beside the sha256 of the copy published, in that folder's README. Reproduction still needs live model endpoints for Experiment 1, a LoRA fine-tuning stack and the Qwen base model for Experiment 2, and torch for Experiment 3.
