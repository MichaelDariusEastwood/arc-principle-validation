...
- Experiments:
  ...
  - Experiment 2 (Representational): Qwen 2.5 3B Instruct + LoRA fine-tuning under capability-only, safety-only, and entangled loss. Both v1 (9 examples, rank 8, 100 iterations) and v2 (295 examples, rank 16, 500 iterations) were run. No runner script in-tree.
  - Experiment 3 (Architectural): PyTorch gated self-modification simulation with LSTM meta-controller and a drag control. Four conditions (static, babylon, eden, drag_control), 3 seeds each = 12 runs. papers/Paper-VIII-The-Load-Bearing-Proof/experiments/gated-self-mod-simulation/src/ contains only an .egg-info directory — the runnable Python source is not committed here. See Gaps.
...
- Hash-pinned: No MANIFEST.json. Nothing SHA-pinned.
- Mirrors/duplicates:
...
- Gaps/confusions found:
  - No runner scripts for Experiments 1, 2, or 3 are committed to the paper tree or root experiments/. experiments/gated-self-mod-simulation/src/ contains only egg-info metadata. Reproduction is not possible from the repository alone.
